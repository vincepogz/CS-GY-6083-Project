...
export async function checkEmailExists(email: string): Promise<boolean> {
  try {
    const result = await pool.query(
      'SELECT email FROM account WHERE email = $1 LIMIT 1',
      [email]
    )
...
export async function authenticateUser(email: string, password: string): Promise<boolean> {
  try {
    ...
    const accountResult = await pool.query(
      'SELECT identity_pubguid FROM account WHERE email = $1',
      [email]
    )
    ...
    const pubguid = accountResult.rows[0].identity_pubguid

    // Step 2: Find active identity record and get prvguid
    const identityResult = await pool.query(
      'SELECT prvguid FROM identity WHERE pubguid = $1 AND active = true',
      [pubguid]
    )
...
export async function getSecurityQuestionsForReset(email: string): Promise<{ pubguid: string; questions: any }> {
  try {
    // Step 1: Get pubguid from account table
    const accountResult = await pool.query(
      'SELECT identity_pubguid FROM account WHERE email = $1',
      [email]
    )
...
export async function updatePassword(email: string, newPassword: string): Promise<void> {
  try {
    ...
    await pool.query(
      'UPDATE login SET pword = $1 WHERE uname = $2',
